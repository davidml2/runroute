...
function getDirection(fromLat, fromLng, toLat, toLng) {
  ...
  if (bearing < 45 || bearing >= 315) return { icon: '⬆️', text: 'Straight' };
  if (bearing < 135) return { icon: '➡️', text: 'Turn Right' };
  if (bearing < 225) return { icon: '⬇️', text: 'U-Turn' };
...
export default function NavigationScreen({ route, session, location, onComplete }) {
  ...
  const [navInstruction, setNavInstruction] = useState({ icon: '⬆️', text: 'Ready', sub: 'Connecting GPS...' });
        ...
        if (routeCoords.length > 0) {
          ...
          if (lookAhead > index) {
            ...
            setNavInstruction({
              icon: dir.icon,
              text: dir.text,
              sub: `In ${Math.round(distToNext)}m`,
            });
          }
        }
      },
    ...
    <div className="screen" style={{ background: 'linear-gradient(160deg, #0C1422, #080D18)', position: 'relative' }}>
      ...
      {isOffRoute && (
        <div style={{
          ...
          <span>⚠️</span>
          <span style={{ fontSize: 13, fontWeight: 700 }}>Off Route</span>
        </div>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '0 20px' }}>
        ...
        <div style={{ position: 'relative', width: 150, height: 150 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, color: 'var(--dim)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Done</div>
            <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'rgba(196,181,253,0.6)', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Time</div>
          ...
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: 'rgba(0,229,195,0.6)', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Pace</div>
        ...
        <div style={{ width: '100%' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: 'var(--dim)' }}>Progress</span>
