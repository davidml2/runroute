...
const terrains = [
  { id: 'park', emoji: '🌳', label: 'Park', color: '#86EFAC' },
  { id: 'riverside', emoji: '🏞️', label: 'River', color: '#7DD3FC' },
  { id: 'urban', emoji: '🏙️', label: 'Urban', color: '#C4B5FD' },
  { id: 'mountain', emoji: '⛰️', label: 'Mountain', color: '#FCA5A5' },
  { id: 'beach', emoji: '🏖️', label: 'Beach', color: '#FFD93D' },
];
...
export default function SetupScreen({ onRecommend }) {
      ...
      (pos) => {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json&accept-language=en`)
          ...
          .then(data => {
            ...
            setCityName(addr.city || addr.town || addr.county || addr.state || 'Current Location');
          })
          .catch(() => setCityName('Current Location'));
      },
      (err) => {
  ...
  const handleSubmit = () => {
    if (!coords) {
      alert('Acquiring GPS location. Please try again shortly.');
      return;
    }
    onRecommend({
      lat: coords.lat,
  ...
  return (
    <div className="screen" style={{ background: 'linear-gradient(160deg, #0D1428, #080D1E)' }}>
      ...
      <div style={{ textAlign: 'center', padding: '40px 20px 10px' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--dim)' }}>AI Running Route Recommendation</p>
      </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>
            {gpsStatus === 'ready' ? `📍 ${cityName}` :
             gpsStatus === 'acquiring' ? 'Acquiring location...' :
             gpsStatus === 'error' ? 'GPS Error - Please check location permissions' : 'Waiting...'}
          </span>
        </div>
      </div>
